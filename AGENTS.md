...
| `filesystem/watcher.rs` | `notify`-based OS watcher: `watch_directories()` watches any set of directories non-recursively, classifies events into `Added`/`Removed`/`Modified`/`Renamed` (drops `Access` noise) and debounces bursts through a private 100 ms debounce thread — `notify-debouncer-mini` was rejected because it reduces every event to `(path, Any)` and would destroy the kind information the GUI routing relies on. **Rename classification:** backends that pair both sides (inotify) emit one event with two paths → `Renamed(from, to)`; Windows delivers the two sides as separate single-path `Name(From)`/`Name(To)` events and FSEvents reports `Name(Any)` once per side, so a single-path side is classified by existence — the side that still exists is `Added` (destination), the vanished side is `Removed` (source). |
