...
**Failure semantics:** whole-packages-dir purges happen ONLY on PGP verification failures (security-motivated). Transient download/size failures delete only this run's artifacts (per-run unique temps via `unique_temp_path`, `<stem>.<label>.<pid>.<counter>`) so a previously staged, verified package from an earlier run survives a network blip. The signature download is capped at 1 MiB (`MAX_SIGNATURE_SIZE`) like the package cap. The package is staged to a unique `.partial` temp then moved into place via `path_utils::atomic_write`; if the rename fails with a stale/locked-destination error kind (`PermissionDenied`, on unix also `AlreadyExists`), the destination is removed and the rename retried once — any other rename failure propagates the ORIGINAL error and removes nothing. The `.partial`/`.sig` temps deliberately lack the `.nupkg` extension so `pre_startup_verify_packages` never scans a leftover temp as a staged package. `UpdateFailed` is surfaced to the user via the status banner (`status-update-failed`), not just logged. The riskiest logic (size caps, version validation, temp naming, rename retry) lives in pure helpers with unit tests, not inside the async download closure.
