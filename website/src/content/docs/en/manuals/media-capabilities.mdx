...
- **Dynamic Dependency:** Codec and container format compatibility depends entirely on the version of `libmpv` and the underlying FFmpeg compilation (e.g. `libavcodec`/`libavformat`) installed on the host system. If `libmpv` is not installed, in-app video playback is disabled but video files are still detected and browsable. Video thumbnails also require mpv. Images and audio are unaffected.
...
_Note: Filtering matches against the filename structure case-insensitively; searching does not query internal file metadata fields (like EXIF tags or audio artists)._
